...
data_features <- data_proc %>%
  ...
  mutate(age_driver = interval(start = driver_dob, end = date)/years(1),
         ...
         wins_driver = slide_dbl(.x = case_when(race_position == 1 ~ 1, TRUE ~ 0),
                                 ...
                                 .before = Inf, .after = -1),
         # Driver Win ratio
         ratio_wins_driver = wins_driver/races_driver,
         # Cumulative podiums
         podiums_driver = slide_dbl(.x = case_when(race_position <= 3 ~ 1, TRUE ~ 0),
                                    .f = ~sum(.x, na.rm = TRUE), 
                                    .before = Inf, .after = -1),
         # Driver Win ratio
         ratio_podiums_driver = podiums_driver/races_driver,
  ...
  mutate(races_constructor = dense_rank(race_id + case_when(race_type == "SPRINT" ~ -0.5, TRUE ~ 0)),
         ...
         wins_constructor = slide_index_dbl(.x = case_when(race_position == 1 ~ 1, TRUE ~ 0),
                                 ...
                                 .f = ~sum(.x, na.rm = TRUE), 
                                 .before = Inf, .after = -1),
         # Ratio wins constructor
         ratio_wins_constructor <- wins_constructor/races_constructor,
         # Cumulative podiums
         podiums_constructor = slide_index_dbl(.x = case_when(constr_driver_index == 1 & race_position <= 3 ~ 1, TRUE ~ 0),
                                               .i = races_constructor,
                                    .f = ~sum(.x, na.rm = TRUE), 
                                    .before = Inf, .after = -1),
         # Ratio podiums constructor
         ratio_podiums_constructor <- podiums_constructor/races_constructor,
         # Cumulative average position
  ...
  mutate(races_driver_circuit = slide_int(.x = race_id, .f = length, 
         ...
         wins_driver_circuit = slide_dbl(.x = case_when(race_position == 1 ~ 1, TRUE ~ 0),
                                 .f = ~sum(.x, na.rm = TRUE), 
                                 .before = Inf, .after = -1),
         # Ratio of wins at circuit
         ratio_wins_driver_circuit = wins_driver_circuit/races_driver_circuit,
         # Cumulative podiums
         podiums_driver_circuit = slide_dbl(.x = case_when(race_position <= 3 ~ 1, TRUE ~ 0),
                                    .f = ~sum(.x, na.rm = TRUE), 
                                    .before = Inf, .after = -1),
         # Ratio of podiums at circuit
         ratio_podiums_driver_circuit = podiums_driver_circuit/races_driver_circuit,
         # Cumulative average position
  ...
  mutate(races_constructor_circuit = dense_rank(race_id + case_when(race_type == "SPRINT" ~ -0.5, TRUE ~ 0)),
         ...
         wins_constructor_circuit = slide_index_dbl(.x = case_when(race_position == 1 ~ 1, TRUE ~ 0),
                                 ...
                                 .f = ~sum(.x, na.rm = TRUE), 
                                 .before = Inf, .after = -1),
         # Ratio constructor wins circuit
         ratio_wins_constructor_circuit = wins_constructor_circuit/races_constructor_circuit,
         # Cumulative podiums
         podiums_constructor_circuit = slide_index_dbl(.x = case_when(constr_driver_index == 1 & race_position <= 3 ~ 1, TRUE ~ 0),
                                               .i = races_constructor,
                                    .f = ~sum(.x, na.rm = TRUE), 
                                    .before = Inf, .after = -1),
         # Ratio constructor podiums circuit
         ratio_podiums_constructor_circuit = podiums_constructor_circuit/races_constructor_circuit,
         # Cumulative average position
...
```{r Single variable distros}
fgh <- data_features %>% filter(race_id == 1045 & race_type == "RACE")
fgh2 <- table_race_qualifying %>% filter(raceId == 1045)
```

### Univariate Analysis - Violins
```{r Univariate Violins}
# Age Driver -----------------------------------------------
data_features %>% 
...
data_features %>% 
  mutate(races_driver = cut_interval(x = races_driver, length = 25)) %>% 
  ggplot() +
...
data_features %>% 
  ...
  labs(x = "Points Won %age Driver", y = "Race Position") +
  theme_vaw_light()
...
data_features %>% 
  ...
  geom_violin(mapping = aes(x = ratio_race_wins_driver, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75)) +
  labs(x = "Races Won %age Driver", y = "Race Position") +
  theme_vaw_light()

# Podium Win Ratio Driver -----------------------------------------------
data_features %>% 
  mutate(ratio_quali_gap_driver = cut_interval(x = ratio_quali_gap_driver/races_driver, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_quali_gap_driver, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75)) +
  labs(x = "Time Gap to Pole Ratio", y = "Race Position") +
  theme_vaw_light()
...
# Qualification time gap to leader ------------------------------
data_features %>% 
  filter(!is.na(ratio_quali_gap_driver) & ratio_quali_gap_driver <= 0.5) %>% 
  mutate(ratio_quali_gap_driver = cut_interval(x = ratio_quali_gap_driver, length = 0.05)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_quali_gap_driver, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75)) +
  labs(x = "Quali Time gap to Pole Ratio", y = "Race Position") +
  theme_vaw_light()
...
# Podium Ratio Constructor -----------------------------------------------
data_features %>% 
  mutate(ratio_race_podiums_constructor = cut_interval(x = podiums_constructor/races_constructor, length = 0.1)) %>% 
  ggplot() +
...
# Races Featured Driver Circuit -----------------------------------------------
data_features %>% 
  mutate(races_driver_circuit = cut_interval(x = races_driver_circuit, length = 3)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = races_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Races Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Points Share Driver -----------------------------------------------
data_features %>% 
  mutate(ratio_points_driver_circuit = cut_interval(x = ratio_points_driver_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_points_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Points Won %age Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Race win Ratio Driver -----------------------------------------------
data_features %>% 
  mutate(ratio_wins_driver_circuit = cut_interval(x = wins_driver_circuit/races_driver_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_wins_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Races Won %age Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Podium Win Ratio Driver -----------------------------------------------
data_features %>% 
  mutate(ratio_podiums_driver_circuit = cut_interval(x = podiums_driver_circuit/races_driver_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_podiums_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Podiums Achieved %age Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean race position Driver -----------------------------------------------
data_features %>% 
  mutate(position_mean_driver_circuit = factor(round(position_mean_driver_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = position_mean_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Race Position Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean Grid Position Driver ----------------------------------------------
data_features %>% 
  mutate(grid_mean_driver_circuit = factor(round(grid_mean_driver_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = grid_mean_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Grid Position Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Points Share Last 5 Driver -----------------------------------------
data_features %>% 
  mutate(ratio_points_l5_driver_circuit = cut_interval(x = ratio_points_l5_driver_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_points_l5_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Points Won Last 5 %age Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean Race Position Last 5 Driver --------------------------------------
data_features %>% 
  mutate(position_l5_driver_circuit = factor(round(position_l5_driver_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = position_l5_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Race Position L5 Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean Grid Position Last 5 Driver --------------------------------------------
data_features %>% 
  mutate(grid_l5_driver_circuit = factor(round(grid_l5_driver_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = grid_l5_driver_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Grid Position L5 Driver Circuit", y = "Race Position") +
  theme_vaw_light()

# Races Featured Constructor Circuit -----------------------------------------------
data_features %>% 
  mutate(races_constructor_circuit = cut_interval(x = races_constructor_circuit, length = 10)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = races_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Races Contructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Points Share Constructor Circuit -----------------------------------------------
data_features %>% 
  mutate(ratio_points_constructor_circuit = cut_interval(x = ratio_points_constructor_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_points_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Points Won %age Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Race win Ratio Constructor Circuit -----------------------------------------------
data_features %>% 
  mutate(ratio_wins_constructor_circuit = cut_interval(x = wins_constructor_circuit/races_constructor_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_wins_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Races Won %age Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Podium Win Ratio Constructor Circuit -----------------------------------------------
data_features %>% 
  mutate(ratio_podiums_constructor_circuit = cut_interval(x = podiums_constructor_circuit/races_constructor_circuit, 
                                                               length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_podiums_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Podiums Achieved %age Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean race position Constructor Circuit -----------------------------------------------
data_features %>% 
  mutate(position_mean_constructor_circuit = factor(round(position_mean_constructor_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = position_mean_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Race Position Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean Grid Position Constructor Circuit ----------------------------------------------
data_features %>% 
  mutate(grid_mean_constructor_circuit = factor(round(grid_mean_constructor_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = grid_mean_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Grid Position Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean DNFs Constructor Circuit -----------------------------------------------------------
data_features %>% 
  mutate(dnf_mean_constructor_circuit = cut_interval(x = dnf_mean_constructor_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = dnf_mean_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "DNF %age Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Points Share Last 5 Constructor Circuit -----------------------------------------
data_features %>% 
  mutate(ratio_points_l5_constructor_circuit = cut_interval(x = ratio_points_l5_constructor_circuit, length = 0.1)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = ratio_points_l5_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Points Won Last 5 %age Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean Race Position Last 5 Constructor Circuit --------------------------------------
data_features %>% 
  mutate(position_l5_constructor_circuit = factor(round(position_l5_constructor_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = position_l5_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Race Position L5 Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Mean Grid Position Last 5 Constructor Circuit --------------------------------------------
data_features %>% 
  mutate(grid_l5_constructor_circuit = factor(round(grid_l5_constructor_circuit), levels = 1:20)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = grid_l5_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "Mean Grid Position L5 Constructor Circuit", y = "Race Position") +
  theme_vaw_light()

# Count DNF Last 5 Constructor Circuit ---------------------------------------------------
data_features %>% 
  mutate(dnf_l5_constructor_circuit = factor(round(dnf_l5_constructor_circuit), levels = 1:5)) %>% 
  ggplot() +
  geom_violin(mapping = aes(x = dnf_l5_constructor_circuit, y = race_position),
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "width") +
  labs(x = "DNFs L5 Constructor Circuit", y = "Race Position") +
  theme_vaw_light()
...
library(jtools)
# Qualification Position Driver -----------------------------------------------
lm(formula = race_position ~ quali_position, 
   data = data_features %>% filter(races_driver > 5)) %>% summ()

# Grid Position Driver -----------------------------------------------
lm(formula = race_position ~ grid_position, 
   data = data_features %>% filter(races_driver > 5)) %>% summ()

# Age Driver -----------------------------------------------
lm(formula = race_position ~ age_driver, 
   data = data_features %>% filter(races_driver > 5)) %>% summ()

# Races Driver -----------------------------------------------
lm(formula = race_position ~ races_driver, 
   data = data_features %>% filter(races_driver > 5)) %>% summ()

# Points Ratio Driver ------------------------------
lm(formula = race_position ~ ratio_points_driver, 
   data = data_features %>% filter(races_driver > 5)) %>% summ()
